    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-xs text-white/60">
          {lowBandwidth
            ? "Auto-compression to 480p enabled (text-only analysis)."
            : "Client-side preview with lightweight analysis."}
        </div>
      </div>
      ...
      {!videoUrl ? (
        ...
        >
          ...
          <div className="flex gap-3">
            ...
            <label className="rounded-xl bg-white px-4 py-2 text-sm font-semibold text-black">
              Camera Capture
              <input
                type="file"
                accept="video/*"
          ...
          <section className="relative rounded-2xl bg-[#0E0E0E] p-4">
            ...
            {lowBandwidth ? (
              <div className="rounded-xl bg-white/5 p-4 text-sm text-white/80">
                Video playback disabled. Analysis below is text-only.
              </div>
            ) : (
              <video
                src={videoUrl}
                ...
                {metrics.map((m) => (
                  <div key={m.label} className="rounded-xl bg-white/5 p-3">
                    ...
                    <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-white/10">
                      <div
                        className="h-full bg-gradient-to-r from-[#9B5FFF] to-[#4DD0FF]"
                        style={{ width: `${m.value}%` }}
                      />
                    </div>
                  </div>
          ...
          <section className="lg:col-span-2 rounded-2xl bg-[#0E0E0E] p-4">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <ReportBlock
                title="Weaknesses"
                items={[
                  "Lateral stability dips on turns",
                  "Minor heel strike on sprints",
                ]}
              />
              <ReportBlock
                title="Dietary recommendations"
                items={[
                  "Add 1g/kg protein daily",
                  "Hydrate with electrolytes post-session",
                  "Include leafy greens and nuts",
                ]}
              />
              <ReportBlock
                title="Suggested drills & recovery tips"
                items={[
              ...
              <ReportBlock
                ...
                items={[
                  "Great streak! Keep the momentum",
                  "You’re 2 points from the next tier",
                ]}
              />
            </div>
