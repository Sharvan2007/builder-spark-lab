  ...
  const notifications = useMemo(
    () =>
      [
        ...
        {
          id: 2,
          text: "Coach feedback available for Sprint Drill.",
          priority: 2,
        },
        {
          id: 3,
          text: "AI flagged inconsistencies in a recent upload.",
          priority: 0,
        },
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        ...
        <section className="ts-white-card p-5 hover-glow">
          ...
          <ul className="space-y-2">
            {notifications.map((n) => (
              <li
                key={n.id}
                className="flex items-center gap-2 rounded-lg bg-black/5 px-3 py-2"
              >
        ...
        <section className="ts-white-card p-5 hover-glow">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { icon: PlayCircle, label: "Sprint" },
              { icon: Bolt, label: "Agility" },
              { icon: Flame, label: "Endurance" },
              { icon: Shield, label: "Balance" },
              { icon: Star, label: "Technique" },
              { icon: Zap, label: "Power" },
            ].map((d) => (
              <div
                key={d.label}
                className="flex flex-col items-center justify-center gap-2 rounded-xl bg-black/5 p-3"
              >
