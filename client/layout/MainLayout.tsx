  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-black/80 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          <div className="flex items-center gap-3">
            ...
            <div className="bg-gradient-to-r from-[#9B5FFF] to-[#4DD0FF] bg-clip-text text-lg font-extrabold text-transparent">
              TalentScout
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <select
              aria-label="Language"
              className="rounded-lg bg-white/5 px-2 py-1.5 text-xs text-white"
            >
              <option>EN</option>
              <option>HI</option>
              <option>BN</option>
          ...
          <div className="no-scrollbar mx-auto flex max-w-7xl items-center gap-4 overflow-x-auto px-4 py-2 text-sm">
            {nav.map((n) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    "rounded-xl px-3 py-1.5 text-white/80",
      ...
      <footer className="border-t border-white/10 py-6 text-center text-xs text-white/50">
        © {new Date().getFullYear()} TalentScout
      </footer>
    </div>
  );
}
